...
----------------------------------------------------------------------
6. Observation Structure (Summary)
----------------------------------------------------------------------
...
- taxonomy: what is wrong
- cause: how it failed (not an interpretation)
- source / channel / substrate: observation path and execution context
- origin: detection point (recorded whenever possible)

Observation is neither a DTO nor a log entry.
It is treated as a fact declaration that constrains interpretation.

----------------------------------------------------------------------
Unified Observation — Draft Field Definition (Design Specification)
----------------------------------------------------------------------

This section defines the complete field set of the unified Observation
model, which will succeed the legacy org.goldenport.observation.Observation.

This definition is authoritative at the design level and intentionally
separates observational facts from interpretation and disposition.
It does NOT prescribe implementation details.


----------------------------------------------------------------------
Core Observation (Required)
----------------------------------------------------------------------

taxonomy
  Category × Symptom pair representing what is factually wrong.
  This is the central axis of error-related observation and is fixed
  prior to any interpretation or handling.

cause
  Describes how the failure occurred (failure mechanism), not why.
  This field captures the observed mode of failure and is independent
  of responsibility or recovery strategy.

source
  Logical source where the observation originates, such as a subsystem,
  component, or architectural module.
  Represents where in the system structure the issue is observed.

channel
  Access or interaction channel through which the observation was
  encountered (e.g., CLI, REST API, batch, internal call).

substrate
  Execution substrate or technical platform on which the observation
  occurred, such as runtime environment or infrastructure layer.

origin
  Detection or confirmation point where the observation was definitively
  established, such as a method, validation boundary, or external
  assertion.


----------------------------------------------------------------------
Observability Core (Required)
----------------------------------------------------------------------
...
location
  Logical or physical execution location where the observation
  occurred, such as node, process, or subsystem.


----------------------------------------------------------------------
Observability Extensions (Optional)
----------------------------------------------------------------------
...
environment
  Environment classification (e.g., production, staging, local).
  Observational only.
...
- The unified Observation model is a successor, not a derivative, of
...
- Observability requirements from the legacy model are preserved,
  ...
  and cause.
- This specification intentionally avoids premature Scala encoding.
  Implementation details will follow once semantics are fully fixed.
...
- Adding severity, retryability, or responsibility
