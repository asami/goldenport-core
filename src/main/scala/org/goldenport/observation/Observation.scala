...
case class Taxonomy(
  ...
  def print = s"${category.name}.${symptom.name}"
...
object Taxonomy {
  enum Category(val name: String, val value: Int) {
    ...
    case Value extends Category("value", 3)

    case Record extends Category("record", 4)

    case Entity extends Category("entity", 5)
    ...
    /** The problem is observed in a resource or reference to a resource.
      ...
      */
    case Resource extends Category("resource", 6)

    case Reference extends Category("reference", 7)
    ...
    /** The problem is observed in the state of a system, component, or domain entity.
      ...
      */
    case State extends Category("state", 8)

    case Security extends Category("security", 9)

    case Operation extends Category("operation", 10)

    case Service extends Category("service", 11)

    case ServiceProvider extends Category("service-provider", 12)

    case Component extends Category("component", 13)

    case SubSystem extends Category("subsystem", 14)
    ...
    /** The problem is observed at the system level and cannot be meaningfully attributed to argument, property, configuration, resource, or state.
      ...
      */
    case System extends Category("system", 15)
    ...
    /** The problem is observed in runtime or environment configuration.
      ...
      */
    case Configuration extends Category("configuration", 16)

    case DataStore extends Category("datastore", 17)

    case Network extends Category("network", 18)

    case OutOfControl extends Category("out-of-control", 99)
  }
  ...
  enum Symptom(val name: String, val value: Int) {
    ...
    case Missing extends Symptom("missing", 3)
    /** A value or element is supplied more than once or where it must not be present. */
    case Redundant extends Symptom("redundant", 4)
    ...
    case DomainValue extends Symptom("domain-value", 7) // reserved; prefer Invalid for new code.
    /** The observed value or state does not satisfy the conditions required to be considered valid or well-formed. */
    case Invalid extends Symptom("invalid", 8)
    /** The observed operation or state is explicitly prohibited by defined rules or constraints. */
    case Illegal extends Symptom("illegal", 9)
    /** An attempt to locate or retrieve a target confirms that the target does not exist. */
    case NotFound extends Symptom("not-found", 10)
    /** The target may exist, but its existence or accessibility cannot be confirmed due to reachability or availability issues. */
    case Unavailable extends Symptom("unavailable", 11)
    /** The observed condition represents a conflict, such as a concurrency conflict or a violation of consistency constraints. */
    case Conflict extends Symptom("conflict", 12)
    /** A reference points to a target that is invalid, inconsistent, or cannot be resolved as a valid reference. */
    case InvalidReference extends Symptom("invalid-reference", 13)
    case Duplicate extends Symptom("duplicate", 14)
    case AuthenticationRequired extends Symptom("authentication-required", 15)
    case PermissionDenied extends Symptom("permission-denied", 16)
    /** The observed data or resource is damaged, inconsistent, or internally broken in a way that prevents correct use. */
    case Corrupted extends Symptom("corrupted", 17)

    // OutOfControl
    case NullPointer extends Symptom("null-pointer", 90)

    case UnreachableReached extends Symptom("unreachable-reached", 91)

    case UninitializedState extends Symptom("uninitialized-state", 92)

    case InvariantViolation extends Symptom("invariant-violation", 93)
    case PreconditionViolation extends Symptom("precondition-violation", 94)
    case PostconditionViolation extends Symptom("postcondition-violation", 95)

    case NotImplemented extends Symptom("not-implemented", 96)

    case ImpossibleState extends Symptom("impossible-state", 97)
  }
...
object Cause {
  ...
  enum Kind(val name: String, val value: Int) {
    case Format extends Kind("format", 1)
    case Limit extends Kind("limit", 2)
    case Policy extends Kind("policy", 3)
    case Capability extends Kind("capability", 4)
    case Permission extends Kind("permission", 5)
    case Guard extends Kind("guard", 6)
    case Relation extends Kind("relation", 7)
    case Conflict extends Kind("conflict", 8)
    case Inconsistency extends Kind("inconsistency", 9)
